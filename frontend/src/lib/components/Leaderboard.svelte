...
{:else}
  <ol>
    {#each models as m, i (m.provider + m.model)}
      ...
      <li class="hairline-b py-2.5 last:border-b-0">
        <div class="flex items-baseline gap-3">
          <span class="numeral w-7 text-sm text-muted">{String(i + 1).padStart(2, '0')}</span>
          <span class="truncate text-sm font-bold">{m.model}</span>
          <span class="microlabel-dim">{m.provider}</span>
          <span class="numeral ml-auto text-sm">{fmtUsd(m.cost_usd, !!m.cost_estimated)}</span>
        </div>
        <div class="mt-1.5 flex items-center gap-3 pl-10">
          <div class="h-1 flex-1 bg-ink-2">
            <div class="h-full" style="width: {((m.cost_usd ?? 0) / maxCost) * 100}%; background: var(--red);"></div>
          </div>
          <span class="microlabel-dim w-20 text-right">{fmtTokens(total)} tkn</span>
          <span class="microlabel-dim w-24 text-right">{perM(m)}/1M</span>
          <span
            class="microlabel-dim w-16 text-right"
