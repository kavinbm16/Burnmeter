<script lang="ts">
  ...
  let { budget, onsave }: { budget: Budget; onsave: (v: number | null) => void } = $props()
  ...
  const monthPct = $derived((budget.days_elapsed / budget.days_in_month) * 100)
...
<div class="flex h-full flex-col">
  ...
  {#if editing || budget.monthly_usd == null}
    <div class="mt-3 flex gap-1">
      ...
      />
      <button class="bg-red px-3 text-xs font-bold text-ink" onclick={save}>SET</button>
    </div>
    {#if budget.monthly_usd == null}
